...
const contexts = new WeakMap<Element, GhostContext>();
...
const GhostFibers = ({
  ...
  useEffect(() => {
    ...
    const geometry = new Triangle(gl as unknown as never);
    const program = new Program(gl as unknown as never, {
      vertex,
      fragment,
      uniforms: {
    ...
    const mesh = new Mesh(gl as unknown as never, { geometry, program });
    ...
    const setSize = () => {
      ...
      res[0] = (gl as unknown as { drawingBufferWidth: number }).drawingBufferWidth;
      res[1] = (gl as unknown as { drawingBufferHeight: number }).drawingBufferHeight;
      render();
    };
  ...
  useEffect(() => {
    ...
    const uniforms = context.program.uniforms as Record<string, { value: number | Float32Array }>;
    ...
    (uniforms.uScale as { value: number }).value = scale;
    (uniforms.uRotation as { value: number }).value = rotation;
    (uniforms.uRotationSpeed as { value: number }).value = rotationSpeed;
    (uniforms.uLayers as { value: number }).value = Math.min(Math.max(Math.round(layers), 1), 10);
    (uniforms.uWaveAmplitude as { value: number }).value = waveAmplitude;
    (uniforms.uWaveFrequency as { value: number }).value = waveFrequency;
    (uniforms.uWaveSpeed as { value: number }).value = waveSpeed;
    (uniforms.uLayerSpeed as { value: number }).value = layerSpeed;
    (uniforms.uTwist as { value: number }).value = twist;
    (uniforms.uTwistFrequency as { value: number }).value = twistFrequency;
    (uniforms.uTwistSpeed as { value: number }).value = twistSpeed;
    (uniforms.uLineFrequency as { value: number }).value = lineFrequency;
    (uniforms.uLineSpacing as { value: number }).value = lineSpacing;
    (uniforms.uLineSharpness as { value: number }).value = lineSharpness;
    (uniforms.uGlowFalloff as { value: number }).value = glowFalloff;
    (uniforms.uGlowIntensity as { value: number }).value = glowIntensity;
    (uniforms.uBrightness as { value: number }).value = brightness;
    (uniforms.uBlueBoost as { value: number }).value = blueBoost;
    (uniforms.uVignette as { value: number }).value = vignette;
    (uniforms.uGrain as { value: number }).value = grain;
    (uniforms.uLightMode as { value: number }).value = lightMode ? 1 : 0;
    context.setFps(fps);
